<template>
	<div class="app-container">
		<div class="filter-container">
			<el-input v-model="listQuery.roleName" :placeholder="$t('pleaseEnter') + $t('roleName')" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
		...
		>
			...
			<el-table-column :label="$t('roleName')" min-width="140" align="center">
				<template slot-scope="{row}">
					<span>{{ row.roleName }}</span>
				</template>
			</el-table-column>
			<el-table-column :label="$t('menubuttonId')" min-width="140" align="center">
				<template slot-scope="{row}">
					<span>{{ row.menubuttonId }}</span>
				</template>
			</el-table-column>
			<el-table-column :label="$t('enabled')" class-name="status-col" width="100">
				<template slot-scope="{row}">
		...
		<el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible">
			<el-form ref="dataForm" :rules="rules" :model="submitData" label-position="left" label-width="200px" style="width: 600px; margin-left:80px;">
				<el-form-item :label="$t('roleId')" prop="category">
					<el-input v-model="submitData.roleId" type="text" />
				</el-form-item>
				<el-form-item :label="$t('roleName')" prop="category">
					<el-input v-model="submitData.roleName" type="text" />
				</el-form-item>
				<el-form-item :label="$t('menubuttonId')" prop="category">
					<el-tree
						:data="treeData"
						show-checkbox
						node-key="id"
...
import { insertEntity,deleteEntity,modifyEntity,getPageList} from '@/api/sysRole.js'
import { getMenuTree} from '@/api/sysMenuButton'
...
export default {
	...
	methods: {
		...
		loadTree(){
			...
			getMenuTree().then((res)=>{
				if(res.code === 200 ){
					...
					const arr = that.submitData.menubuttonId.split(',')
		...
		updateData(){
			...
			const str = checkArr.join(',')
			this.submitData.menubuttonId = str
